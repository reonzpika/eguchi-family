...
| 2026-03-09 | Idea chatbot prompt upgrade (4-phase, rules, stance). | Shared prompts in `src/lib/idea-chat-prompts.ts` (IDEA_CHAT_SYSTEM_PROMPT_BASE + IDEA_CHAT_MESSAGE_PROMPT_TAIL); start and message routes use them. New spec: 基本スタンス, 会話開始, AIの振る舞い (知識で要約＋注記), 4フェーズ, 会話ルール, 出力形式. Two E2E tests skipped: header hide-on-scroll (ChatHeader does not implement it); ideas list プロジェクトに昇格 menu (portal/timing). |
| 2026-03-21 | Engawa hub overhaul: tools-first home, missions, threads, plaza. | Migrations `20250321000000_family_hub.sql`, `20250321001000_hub_notification_types.sql`; `/api/hub/*`; `docs/hub-overview.md`; BottomNav adds ツール; `sendNotification` sends web push when `send_push` and VAPID set. |
| 2026-03-21 | Stitch Strategy A: one six-tab bar (ホーム・ミッション・ツール・アイデア・プロジェクト・設定); home had duplicate nav. | `StitchAppBar` (AIMasterclass + avatar + NotificationBell + help); `AppChrome` uses `max-w-7xl` for all shell routes; `BottomNav` six tabs; `/` drops inline nav; `/ideas` and `/projects` list pages use editorial framing and chips; settings links to `/menu`; ideas FAB uses `aria-label`「新しいアイデア」; `Header` re-exports `StitchAppBar`. |
| 2026-03-21 | Hub `GET /api/hub/tools` and `GET /api/hub/missions` return 500 when `family_*` tables are missing; long `page.goto` route loops starve dev server. | `npm run db:push` applies `20250321000000_family_hub.sql`, etc.; `tests/routes.spec.ts` smoke-tests pages via HTTP GET (no browser) so parallel E2E stays stable; authenticated block checks key hub APIs after sign-in. |
...
| Family AI Hub        | Home `/`, `/tools`, `/tools/new`, tool detail, missions, threads, `/discussions` plaza, `/api/hub/*`, AI enrich on add-tool, notifications hub_* types, push via `sendNotification` when VAPID set | -                                             |
